...
class P2P_Widget extends scbWidget {
	...
	function __construct() {
		parent::__construct( 'p2p', __( 'Posts 2 Posts', EMD_P2P_TEXTDOMAIN ), array(
			'description' => __( 'A list of posts connected to the current post', EMD_P2P_TEXTDOMAIN )
		) );
	}
	...
	function form( $instance ) {
		...
		echo html( 'p', $this->input( array(
			'type' => 'text',
			'name' => 'title',
			'desc' => __( 'Title:', EMD_P2P_TEXTDOMAIN )
		), $instance ) );
		...
		echo html( 'p', $this->input( array(
			...
			'name' => 'ctype',
			'values' => $ctypes,
			'desc' => __( 'Connection type:', EMD_P2P_TEXTDOMAIN ),
			'extra' => "style='width: 100%'"
		), $instance ) );

		echo html( 'p',
			__( 'Connection listing:', EMD_P2P_TEXTDOMAIN ),
			'<br>',
			$this->input( array(
				'type' => 'radio',
				'name' => 'listing',
				'values' => array(
					'connected' => __( 'connected', EMD_P2P_TEXTDOMAIN ),
					'related' => __( 'related', EMD_P2P_TEXTDOMAIN )
				),
			), $instance )
		);
	}
